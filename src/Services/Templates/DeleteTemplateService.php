...
{
    public function handle(WabaAccountData $wabaAccountData, string $templateId): void
    {
        try {
            Http::withToken($wabaAccountData->appJwt)
                ->acceptJson()
            ...
            Log::error('Failed to delete template (' . $templateId . ') on account ' . $wabaAccountData->waba_id . ' : ' . $exception->getMessage());
            Log::error($exception);
        }
    }
}